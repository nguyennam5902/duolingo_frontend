               ...
               {loading ? <SyncOutlined spin className="d-flex justify-content-center display-1 text-danger p-5" /> : <div
                  ...
                  {current == 0 && <>
                     ...
                     {data.listenQuestions.slice(8, 20).map((question, index) => {
                        const tmpI = index + 8
                        return <div>
                           <p id={`listen_${tmpI}`}>{`${tmpI + 1}. `}{question.question}</p>
                           {question.answers.map(answer => {
                              return <div>
                     ...
                     {data.readingData.map((obj, i) => <>
                        ...
                        <div style={{ width: '100%', fontSize: '16px' }}>
                           ...
                           {obj.questions.map((q, j) => {
                              return <div>
                                 <p>{`${10 * i + j + 1}. ${q.question}`}</p>
                                 {q.answers.map(answer => {
                                    return <div>
                                       <input disabled type="radio" name={`reading_${10 * i + j}`} value={answer}
                                          checked={data.readingChoices[10 * i + j] == answer} />&nbsp;{answer}&nbsp;
                  ...
                  {current == 2 && <div style={{ display: "flex", flexDirection: 'column', alignItems: 'center' }}>
                     ...
                     <h5>Thời gian: 60 phút</h5>
                     <h5>Số câu hỏi: 2</h5>
                     {data.writingData.sort((a, b) => a.taskID.type - b.taskID.type).map(obj => <div style={{
                        fontSize: '16px'
                     }}>
                        <h5>Task {obj.taskID.type}</h5>
                        <p>{parse(obj.taskID.text.replace(/\n/g, "<br>"))}</p>
                        <h2>Student answer</h2>
                        <textarea defaultValue={obj.answer} disabled={true} style={{
                           width: '100%'
                        }}></textarea>
